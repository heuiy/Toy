...
def select_pdf_file():
   ...
   for idx, file in enumerate(pdf_files):
       print(f"{idx + 1}. {file}")
   choice = int(input("배치 번호를 선택하세요: ")) - 1
...
def add_watermark_and_address(input_pdf_path, current_document_number, total_documents, address, skip_address):
   reader = PdfFileReader(input_pdf_path)
   writer = PdfFileWriter()
   total_pages = reader.getNumPages()
...
#       can.drawString(237, 49, page_number_text)
...
#           can.rect(250, 400, 500, 30, fill=True, stroke=False)
#           can.rect(210, 420, 500, 30, fill=True, stroke=False)
#           can.rect(218, 409, 500, 15, fill=True, stroke=False)
#           can.rect(218, 440, 500, 15, fill=True, stroke=False)
           can.rect(202, 455, 500, 20, fill=True, stroke=False)        # 240530 주소 밖 하얀색 배경 위치 수정
           can.setFillColor(black)  # 글자색 검정으로 설정
#           can.drawString(205, 460, address)  # 주소 추가
#           can.drawString(218, 409, address)  # 주소 추가
#           can.drawString(218, 440, address)  # 주소 위치 수정
           can.drawString(202, 459, address)  # 주소 위치 수정         # 240530 주소 위치 수정
...
def select_address():                                                                # consignee address 수정 가능!!!!!!
   addresses = [
#       "Merck Sharp & Dohme Saúde Animal Ltda. Vinhedo - SP - Brazil",
       "RUC: 20109333159 AV. DE LAS ARTES NORTE NRO. 310, SAN BORJA, LIMA - PERU",
       "ALLMED FZCO.   P.O. BOX No. 261257 JAFZA, Dubai, U.A.E.",
#       "옵션 21",
#       "여기도 추가!!!!",
       "SMARK FZE JEBAL ALI JAFZA SOUTH LIU10, BD 06, P.O. Box 18076, DUBAI, UAE",
       "ZALAND SARSABZ LTD.  1st floor, Kabul Plaza Jadai Maiwand closed to kochi Barana, Kabul Afghanistan",
       "Av. San Jerónimo #369, Col. La Otra, Del. Alvaro Obregón, C.P 01090, Ciudad de México, México",
       "남아공, 케냐 : MDM 주소 와 실수령지 주소 동일. 주소 수정 불필요함"
   ]
   print("사용할 주소를 선택하세요:")
   ...
   skip_address = (choice == len(addresses) - 1)
   return addresses[choice], skip_address
...
def main():
   ...
   while current_document_number <= total_documents:
       ...
       selected_address, skip_address = select_address()

       temp_pdf_file = os.path.join("D:\\#.Secure Work Folder\\Shipping Mark\\temp", "temp.pdf")
       with open(temp_pdf_file, "wb") as temp_pdf:
           writer.write(temp_pdf)

       writer_with_watermark_and_address = add_watermark_and_address(temp_pdf_file, current_document_number, total_documents, selected_address, skip_address)
