...
def copy_pages(input_pdf_path, first_copies):
    ...
    # 2페이지짜리 PDF인 경우, 첫 번째 페이지를 (first_copies - 1)번 + 두 번째 페이지 1번 = 총 first_copies장
    if total_pages == 2:
        first_copies -= 1
...
def add_watermark_and_address(input_pdf_path, current_document_number, total_documents, batch_number, skip_address=False):
    ...
    qr_image_path = r"D:\#.Secure Work Folder\Shipping Mark\qr\my_qrcode.png"  # QR 코드 이미지 파일 경로
    reader = PdfFileReader(input_pdf_path)
    writer = PdfFileWriter()
    total_pages = reader.getNumPages()
    ...
    for i in range(total_pages):
        ...
        # 주소 추가(건너뛰지 않도록 skip_address=False일 때만 수행)
        if not skip_address:
        ...
        # QR 코드 이미지 추가 (위치와 크기는 임의 지정)
        # x=450, y=700 정도 위치에 width=80, height=80 짜리 이미지 예시
        can.drawImage(qr_image_path, 450, 700, width=80, height=80)
...
def main():
    ...
    # 개별적으로 만든 PDF들을 하나로 합쳐서 최종 combined PDF 생성
